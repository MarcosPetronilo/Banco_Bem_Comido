        ...
        try {
            ...
            if (subject != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                if (userOpt.isPresent() && jwtService.isTokenValid(jwt, subject)) {
                    ...
                    log.debug("Authenticated user {} with role={}", subject, roleName);
                } else {
                    log.debug("JWT invalid or user not found for subject={}", subject);
                }
            }
        } catch (io.jsonwebtoken.JwtException e) {
            log.debug("Failed to parse/validate JWT: {}", e.getMessage());
        }
